---
name: design-system
description: "PAX(preview) · 테넌트 디자인 시스템(발행된 컴포넌트·토큰) 적용 — UI 컴포넌트·페이지·화면을 만들 때 list_design_components 로 존재를 확인하고, 있으면 이 스킬의 규칙대로 완성 코드·토큰을 가져와 그대로 삽입한다. \"디자인 시스템\", \"우리 컴포넌트로\", \"브랜드 스타일로\" 요청 시 필수. 발행된 디자인 시스템이 없으면 즉시 빠져나와 일반 생성."
---
